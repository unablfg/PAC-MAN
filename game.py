...
class Game:
    ...
    def __init__(self, screen):
        ...
        self.packman_sprite = pygame.sprite.Group()
        self.ghosts_sprites = pygame.sprite.Group()
        self.boosts_sprites = pygame.sprite.Group()
        self.coins_sprites = pygame.sprite.Group()
        self.fruits_sprites = pygame.sprite.Group()
        self.packman = PackMan(self.packman_sprite)
        self.packman.parent_game = self
        self.blinky = Blinky(self.ghosts_sprites)
        self.pinky = Pinky(self.ghosts_sprites)
        self.inky = Inky(self.ghosts_sprites)
        self.clyde = Clyde(self.ghosts_sprites)
        ...
        for i in range(self.width):
            for j in range(self.height):
                if self.board[j][i] == 1:
                    Coin(i, j, self.coins_sprites)
                if self.board[j][i] == 2:
                    Boost(i, j, self.boosts_sprites)
    ...
    def game_over(self, win):
        ...
        while running:
            ...
            if win:
                self.show_text(110, 'YOU WIN!', (40, 60))
            else:
                self.show_text(100, 'OOPS...', (100, 60))
            self.show_text(90, "GAME OVER", (35, 250))
            self.show_text(70, "1. RESTART", (90, 400))
            self.show_text(70, "2. FINISH", (90, 550))
    ...
    def render(self):
        ...
        self.boosts_sprites.draw(self.screen)
        if self.fruit is not None:
            self.fruits_sprites.draw(self.screen)
        self.ghosts_sprites.draw(self.screen)
        self.packman_sprite.draw(self.screen)
        for i in range(self.width):
            for j in range(self.height):
        ...
        if (self.fruit_type < 3 and self.points - self.points_received_from_fruits > self.fruit_levels[
            self.fruit_type]):
            self.fruit = Fruits(choice([14, 13]), 17, self.fruit_type, self.fruits_sprites)
            self.fruit_type += 1
        ...
        if pygame.sprite.spritecollide(self.packman, self.coins_sprites, True):
            self.points += 10
            self.items_count -= 1
        # Столкновение пакмана с энерджайзером
        if pygame.sprite.spritecollide(self.packman, self.boosts_sprites, True):
            self.points += 50
            self.items_count -= 1
        ...
        if pygame.sprite.spritecollideany(self.packman, self.fruits_sprites):
            self.points += self.fruit.points
            self.points_received_from_fruits += self.fruit.points
            self.fruits_sprites.empty()
            self.fruit = None
            pygame.time.set_timer(self.FRUIT_EXISTENCE_TIMER, 0)
        # Столкновение пакмана с призраком
        if pygame.sprite.spritecollideany(self.packman, self.ghosts_sprites):
            if pygame.sprite.spritecollideany(self.packman, self.ghosts_sprites).mode != 'RETURNING_HOME':
                if self.packman.boost_mode:
                    pygame.sprite.spritecollideany(self.packman, self.ghosts_sprites).mode = 'RETURNING_HOME'
                    self.eaten_ghosts += 1
                    self.points += 2 ** self.eaten_ghosts * 100
                else:
                    self.lives -= 1
                    if self.lives:
                        self.ghosts_sprites.empty()
                        self.blinky = Blinky(self.ghosts_sprites)
                        self.pinky = Pinky(self.ghosts_sprites)
                        self.inky = Inky(self.ghosts_sprites)
                        self.clyde = Clyde(self.ghosts_sprites)
